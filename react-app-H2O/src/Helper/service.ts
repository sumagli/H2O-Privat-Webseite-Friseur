    ...
    const fetchImages = async () => {
      try {
        ...
        const imageUrls = response.data.story.content.Bilder.map(
          (item: any) => ({
            url: item.Bild.filename,
            text: item.BildText,
            showText: item.LandingpageTextAnzeigen,
          })
        );
...
export const useVitaIris = () => {
  ...
  React.useEffect(() => {
    const fetchImages = async () => {
      try {
        const response = await axios.get(
          "https://api.storyblok.com/v2/cdn/stories/lebenslauf/lebenslauf-iris?version=draft&token=EH3r7BBXGHB0339e79k4FQtt&cv=1709754988"
        );
